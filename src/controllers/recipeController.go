...
// 	rfArgs := make([]models.RecipeFood, 0)
// 	for rows.Next() {
// 		var recipe_food models.RecipeFood
...
// 	var recipe_foods []models.RecipeFood
// 	for rows.Next() {
// 		var recipe_food models.RecipeFood
// 		err = rows.Scan(&recipe_food.ID, &recipe_food.RecipeId, &recipe_food.RecipeName, &recipe_food.RecipeDescription, &recipe_food.FoodId, &recipe_food.FoodName, &recipe_food.UseAmount, &recipe_food.FoodUnit, &recipe_food.RecipeMakingMethod)
// 		if err != nil {
...
// 	// recipeFoodArrayへとリクエストボディに受け取ったjsonデータを配列として受け取る
// 	var recipeFoodArray models.RecipeFoodArray
// 	err := json.NewDecoder(r.Body).Decode(&recipeFoodArray)
// 	if err != nil {
// 		log.Fatal(err.Error())
// 	}
...
// 		if now == time.Date(now.Year(), now.Month(), now.Day(), 10, now.Minute(), now.Second(), now.Nanosecond(), loc) {
...
// 			expirationFoodArgs := make([]models.Food, 0)
// 			for foodRows.Next() {
